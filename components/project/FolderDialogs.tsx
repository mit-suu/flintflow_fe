...
import { FOLDER_COLORS, FOLDER_COLOR_ORDER } from "./FolderCard";
...
function FolderDialog({ target, folders, onClose, onDone }: FolderDialogsProps & { target: FolderDialogTarget }) {
  ...
  const [color, setColor] = useState<FolderColor>(initial?.color ?? "violet");
        ...
        >
          <fieldset className="flex flex-col gap-1.5">
            ...
            {options.map((o) => (
              <label
                key={o._id ?? "none"}
                className={`flex items-center gap-2.5 px-3 h-10 rounded-[10px] border cursor-pointer text-[13px] has-[:focus-visible]:ring-2 has-[:focus-visible]:ring-primary ${
  ...
  const unchanged = !isCreate && name.trim() === target.folder.name && color === target.folder.color;
  return (
