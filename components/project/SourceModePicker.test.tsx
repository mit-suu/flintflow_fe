...
vi.mock("@/lib/project-source-mode", async (importOriginal) => {
  ...
  return {
    ...actual,
    SOURCE_MODE_OPTIONS: actual.SOURCE_MODE_OPTIONS.map((o) => (o.value === "edit_srs" ? { ...o, status: "ready" } : o)),
  };
});

function Controlled({ onChange }: { onChange?: (m: ProjectSourceMode) => void }) {
  const [value, setValue] = useState<ProjectSourceMode | null>(null);
  return (
    <SourceModePicker
      value={value}
      onChange={(m) => {
...
describe("SourceModePicker (UC-13)", () => {
  ...
  it("click chọn thẻ, aria-checked theo", () => {
    ...
    expect(onChange).toHaveBeenCalledWith("edit_srs");
