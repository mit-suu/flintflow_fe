...
const account = vi.hoisted(() => ({ token: null as string | null, patchMe: vi.fn(async () => ({ data: null, error: null })) }));
vi.mock("@/lib/api/token-store", () => ({ getStoredAuthToken: () => account.token }));
vi.mock("@/lib/api/users", () => ({ patchMe: account.patchMe }));
...
describe("LocaleSwitcher", () => {
  ...
  it("chưa đăng nhập ⇒ chỉ ghi cookie, không gọi BE", () => {
    renderWithIntl(<LocaleSwitcher />, "vi");
    fireEvent.click(screen.getByRole("button", { name: "English" }));
    expect(account.patchMe).not.toHaveBeenCalled();
  });
  ...
  it("đã đăng nhập ⇒ lưu luôn vào tài khoản (PATCH /users/me)", () => {
    ...
    renderWithIntl(<LocaleSwitcher />, "vi");
    fireEvent.click(screen.getByRole("button", { name: "English" }));
    expect(account.patchMe).toHaveBeenCalledWith({ locale: "en" });
    expect(document.cookie).toContain("NEXT_LOCALE=en");
  });
