...
describe("ReadinessSummary", () => {
  it("null ⇒ hiện trạng thái đang tải, không có nội dung readiness", () => {
    renderWithIntl(<ReadinessSummary readiness={null} />);
  ...
  it("hiện '72% accepted · 4 chờ duyệt lại · 2 cờ đỏ' — chỉ một chỗ dùng '%' (mô tả tình trạng, không phải điều kiện chốt)", () => {
    ...
    const { container } = renderWithIntl(<ReadinessSummary readiness={readiness} />);
  ...
  it("stale = 0 thì không hiện 'mục cũ'", () => {
    renderWithIntl(<ReadinessSummary readiness={{ accepted_pct: 50, awaiting_reaccept: 0, red_open: 0, stale: 0 }} />);
    expect(screen.queryByText(/mục cũ/)).not.toBeInTheDocument();
  });

  it("stale > 0 thì hiện 'N mục cũ'", () => {
    renderWithIntl(<ReadinessSummary readiness={{ accepted_pct: 50, awaiting_reaccept: 0, red_open: 0, stale: 3 }} />);
    expect(screen.getByText("3 mục cũ")).toBeInTheDocument();
  });

  it("red_open = 0 không dùng màu cảnh báo cho '0 cờ đỏ'", () => {
    renderWithIntl(<ReadinessSummary readiness={{ accepted_pct: 100, awaiting_reaccept: 0, red_open: 0, stale: 0 }} />);
