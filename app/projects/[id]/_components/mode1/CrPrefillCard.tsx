    ...
    return (
      <div role="status" className="bg-[#F2F1FB] border border-[#DCD8F0] rounded-[14px] p-3.5 flex flex-col gap-2 text-[12.5px] text-[#554DB0]">
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Link href={`/projects/${projectId}/change-requests/${created.cr_id}`} className="px-3 py-1.5 rounded-[8px] bg-[#6A62C4] text-white font-bold">
            Mở {created.cr_id}
          </Link>
        </div>
      </div>
  ...
  return (
    <div role="status" className="bg-[#F2F1FB] border border-[#DCD8F0] rounded-[14px] p-3.5 flex flex-col gap-2 text-[12.5px] text-[#554DB0]">
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Link href={crPrefillHref(projectId, { ...prefill, source: "verbal" })} className="px-3 py-1.5 rounded-[8px] bg-[#6A62C4] text-white font-bold">
          Tạo change request
        </Link>
      </div>
    </div>
