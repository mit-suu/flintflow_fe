...
export default function ChangePanel({ projectId, getBaseVersion, getLatestSeq, onApplied, onClose, seed }: ChangePanelProps) {
  const t = useTranslations("workspace.change");
    ...
    <aside className="w-[380px] flex-none bg-[#FAF9F7] border-l border-[#ECEAE5] flex flex-col overflow-hidden z-10" aria-label="Change panel">
      <div className="p-3.5 border-b border-[#ECEAE5] bg-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-extrabold text-[13px] text-[#191817]">{t("title")}</h3>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        ...
        {clarification && (
          <div className="bg-[#F4F3FE] border border-[#DDD9F6] rounded-[12px] p-3 text-[11.5px] text-[#3B34B0]">
            <div className="font-bold mb-1">{t("clarify")}</div>
            {clarification}
          </div>
        )}
        ...
        <div className="flex flex-col gap-1.5 border-t border-[#ECEAE5] pt-3">
          ...
          {showHistory && (
            <div className="flex flex-col gap-1 max-h-56 overflow-y-auto">
              {historyLoading && <div className="text-[11px] text-[#A8A49C] italic">{t("loading")}</div>}
              {!historyLoading && history.length === 0 && (
                <div className="text-[11px] text-[#A8A49C] italic">{t("noHistory")}</div>
              )}
              {history.map((change) => (
        ...
        <div className="flex flex-col gap-1.5 border-t border-[#ECEAE5] pt-3">
          <button
            ...
            className="self-start text-[11px] font-bold text-[#4F46E5] hover:underline cursor-pointer"
          >
            {showTraceability ? t("hideTrace") : t("showTrace")}
          </button>
