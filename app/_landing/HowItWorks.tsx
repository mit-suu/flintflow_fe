...
import { PHASE_GROUPS } from "./content";
 ...
 * Nhãn giai đoạn lấy từ step registry qua `tPhase`, không chép tay vào messages.
 */
  ...
  return (
    <section id="cach-hoat-dong" aria-labelledby="how-title" className="relative scroll-mt-24 bg-surface-container-lowest px-4 pb-20 pt-0 sm:px-6 lg:pb-28">
      ...
      <div className="relative mx-auto max-w-[1200px] pt-16 lg:pt-24">
        <SectionHeading id="how-title" eyebrow={t("how.eyebrow")} title={t("how.title")} subline={t("how.subline")} />
        ...
        <motion.ol {...inView} variants={stagger(0.08)} className="mt-12 grid gap-3 md:grid-cols-2 lg:grid-cols-[2fr_2fr_4fr_2fr]">
          {PHASE_GROUPS.map((group, i) => {
            const title = t(`how.groups.${group.key}.title`);
            return (
              <motion.li key={group.key} variants={fadeUpThen(0.04, 0.2)} className={`flex flex-col rounded-card p-5 sm:p-6 ${TONE_FILL[group.tone].body}`}>
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="text-[20px] font-bold tracking-tight text-on-surface">{title}</h3>
                  <span className="font-mono text-[12px] font-bold text-on-surface/50">0{i + 1}</span>
                </div>
                <p className="mt-2 text-[13.5px] leading-relaxed text-on-surface-medium">{t(`how.groups.${group.key}.body`)}</p>
                <ul className="mt-6 flex flex-wrap gap-1.5 pt-1" aria-label={t("a11y.groupPhases", { group: title })}>
